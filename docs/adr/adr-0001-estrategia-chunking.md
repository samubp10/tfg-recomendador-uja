...
- **Ámbito:** fragmentación de la colección documental del Recomendador UJA
- **Corpus sobre el que se decide:** curso 2026-27 de la EPSJ — 528 asignaturas,
  288 guías docentes, 8 bloques de salidas profesionales
- **Instrumento de medida:** las 50 preguntas anotadas de
  `eval/preguntas_evaluacion.json`
- **Depende de:** ADR-0003 (modelo de incrustaciones), porque medir la calidad de
  un troceado exige vectorizar
...
Ventanas de tamaño fijo sobre el texto. Es la más simple de implementar y no
necesita conocer la estructura del documento. Corta a mitad de frase y, si se aplica
sobre la colección entera en lugar de dentro de cada unidad, puede juntar dos
asignaturas en un mismo fragmento. El solape duplica contenido y regala
oportunidades de ser recuperado.

### Opción B — Corte estructural por unidad *(elegida)*
...
autocontenido con nombre, tipo, ECTS y titulación. Respeta la restricción de diseño
**por construcción** y corta en fronteras del propio texto.
...
el propio modelo detecta un cambio de tema. Es la alternativa conceptualmente más
atractiva y la que más se cita en la literatura divulgativa.
...
Las tres primeras se miden. La cuarta queda descartada por la distribución de
tamaños, que es un dato y no una opinión.

---

## Método: una rejilla de 45 configuraciones
...
sistemáticamente todas las combinaciones de los parámetros en juego.

### Diseño

`scripts/experimento_fragmentacion.py`, sobre `data/grados.json`, con las 50
preguntas del conjunto de evaluación y el modelo del ADR-0003, en CPU.
...
### 1. El tamaño manda; la estrategia casi no
...
### 2. El factor de confusión que impide quedarse con la primera fila
...
Por eso no se elige la fila con mejor número. La comparación que sí aísla el efecto
es a **igualdad de número de fragmentos**:
...
### 3. A igualdad de tamaño, las tres estrategias son indistinguibles
...
|---|---|---:|---:|---:|---:|
| Longitud | solape 20 % | 1.477 | **0,935** | 0,965 | **0,974** |
...
Con un conjunto anotado por una sola persona, diferencias de ese orden no se
distinguen de lo que movería otra anotación: **una pregunta de cincuenta vale 0,020**.

### 4. El truncado, que no depende de ninguna métrica discutible
...
A partir de 1.500 el modelo deja de leer parte del fragmento **sin avisar**: no falla,
...
2. **La Opción C queda medida y descartada.** No es peor: es que **no es mejor**, y
   ...
   a calcular incrustaciones *antes* de fragmentar, de modo que el fragmentador
...
**Decisión: deduplicar dentro del fragmentador.** Las guías se agrupan y cada grupo
produce una sola unidad que arrastra la lista de titulaciones en las que se imparte;
de ahí que los campos `grados` y `codigos` de un fragmento sean listas y no cadenas.
`grados.json` permanece intacto y fiel a la fuente: la deduplicación es una
transformación de representación en el índice, no una pérdida de datos.
...
## Aplicación: lo que produce sobre la colección real
...
**Efecto de la deduplicación:** los fragmentos de guía son 1.193 frente a los **3.631**
...
- **Cincuenta preguntas anotadas por una sola persona**, que es además quien
  construyó el sistema. Una diferencia de una pregunta vale 0,020: las diferencias de
  ese orden que aparecen en la tabla **no son distinguibles** de lo que movería otra
  anotación.
- **RU@K no es del todo inmune al troceo.** Una unidad partida en más fragmentos
  ocupa más huecos del top-K. La decisión se apoya en la comparación a igualdad de
  fragmentos y en el recuento de truncados, no en el orden bruto de la tabla.
- **No se ha medido el efecto sobre la generación**, que es donde la fragmentación
...
- **El tamaño mínimo no se barrió.** Es el único parámetro que sigue sin validación
  experimental propia.
- **El espacio conjunto no se ha explorado.** Esta rejilla se midió con el modelo de
  incrustaciones ya fijado, y la comparación de modelos del ADR-0003 se midió con un
  troceado fijo. Ninguno de los dos experimentos recorre las dos dimensiones a la vez,
  de modo que no puede descartarse que otra combinación fuera mejor que la suma de las
  dos decisiones por separado.
- **Sin solape entre fragmentos.** La rejilla mide el solape del 10 % y del 20 % en la
  estrategia de longitud, pero no en la elegida: no se ha comprobado si un solape
  ayudaría al corte estructural.
...
> **Reejecutada el 13/08/2026 con la estrategia de longitud fija corregida.**
> `_fijo()` descartaba el mínimo de fragmento que se le pasaba y no fusionaba el
> residuo de la última ventana, de modo que las configuraciones con solape dejaban
> una cola corta suelta. La rejilla se ha vuelto a ejecutar entera —las 45, no solo
> las afectadas— y la tabla de abajo es la de esa ejecución.
>
> **Qué cambia:** solo filas de la estrategia `fijo` **con solape**. Las diez de
> solape 10 % y 20 % pierden entre 7 y 48 fragmentos, que son precisamente las colas
> que ahora sí se fusionan; las de solape 0 % no cambian, porque sin solape las
> ventanas encajan y el residuo casi no aparece. En los máximos de 1.200, 1.500 y
> 1.800 cambian además algunas métricas de esas mismas filas.
>
> **Qué NO cambia, y es lo que sostiene esta decisión:** en el máximo de 900, que es
> el adoptado, las nueve configuraciones dan **exactamente las mismas RU@1, RU@3 y
> MRR** que en la ejecución original. Solo se mueven el recuento de fragmentos de las
> dos con solape y las métricas por fragmento, que dependen de en cuántos trozos se
> parte cada unidad. El orden de las estrategias, la elección del máximo y la
> conclusión de este ADR se mantienen sin tocar nada.
...
`intfloat/multilingual-e5-small` (ventana de 512 *tokens*), en CPU. Ordenada por
