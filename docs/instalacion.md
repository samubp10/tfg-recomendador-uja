...
La evidencia y sus límites están en [el registro de IT-54](experimentos/it54-instalacion.md).
...
en la [documentación de Ollama para Docker](https://docs.ollama.com/docker).
Comprobar `docker version`. Los puertos locales 8000 y 11454 deben estar libres.
Los comandos del equipo anfitrión siguientes son para **PowerShell**; los de
la sesión del contenedor son para **sh**. No se necesita Node.js: la interfaz
...
en `127.0.0.1:11434`. El volumen conserva los pesos del LLM. Los datos, el índice
y la caché de incrustaciones quedan en el contenedor de la aplicación y sobreviven
a `docker stop`, pero se perderían al eliminar ese contenedor.

En la sesión **sh** recién abierta:
...
`constraints.txt` fija las dependencias; PyTorch se instala primero desde el
repositorio CPU para que las incrustaciones no necesiten CUDA. Ollama utiliza
la GPU por separado. `pip check` debe terminar sin dependencias rotas.

## 3. Configurar las incrustaciones y la base vectorial

En la misma sesión **sh**, descargar una vez el modelo elegido:
...
Ambas llamadas deben imprimir `384`. Se usa `intfloat/multilingual-e5-small`,
guardado en `/root/.cache/huggingface`. La segunda llamada comprueba la carga
...
[documentación de Hugging Face](https://huggingface.co/docs/huggingface_hub/en/package_reference/environment_variables).
El código añade `passage: ` a los documentos y `query: ` a las preguntas.

Crear el corpus completo y el índice, todavía en **sh**:
...
de base de datos adicional. El indexador crea `data/indice_lance`, con la tabla
`chunks_epsj`, y reconstruye la tabla al repetirlo. No cambiar el modelo de
incrustaciones solo en uno de los dos extremos, indexación o consulta.

Salir al anfitrión con `exit`.

## 4. Descargar el LLM y arrancar la aplicación
...
`ollama list` debe mostrar `gemma3:12b`. El contenedor ya ejecuta el servicio;
no lanzar otro `ollama serve`. La primera descarga falló por un tiempo de
espera agotado en el alojamiento de los pesos; al recuperar la conexión,
repetir `ollama pull gemma3:12b` terminó correctamente. No cambiar de modelo
para dar por superada esta comprobación.

Abrir **http://127.0.0.1:8000**. `socat` permite llegar al servidor, que escucha
solo en el bucle local del contenedor. El puerto publicado del anfitrión sigue
limitado a `127.0.0.1`. No publicar el servicio en una interfaz de red externa.
La aplicación usa por defecto `gemma3:12b`, las incrustaciones anteriores y
`data/indice_lance`; no requiere un fichero `.env` ni claves de servicios remotos.

## 5. Comprobar y volver a arrancar
...
Para conservar la respuesta HTTP, desde otra ventana de **PowerShell**:
...
HTTP 200 no basta: hay que revisar el contenido y el suceso final `fin`.
...
Repetir
solo los dos comandos de `socat` y del servidor del apartado 4. No volver a
