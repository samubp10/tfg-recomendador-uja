...
titulaciones para la Escuela Politécnica Superior de Jaén (EPSJ) de la Universidad de Jaén (UJA): un agente conversacional basado en procesamiento del lenguaje natural (PLN) que asiste a los estudiantes de secundaria proporcionando respuestas precisas, contextualizadas y en lenguaje natural sobre las titulaciones del centro.

El sistema se fundamenta en una arquitectura de Recuperación Aumentada por Generación (\glr{RAG}{RAG}, por sus siglas en inglés: \textit{Retrieval-Augmented Generation}), que permite a un modelo de lenguaje grande (\glr{LLM}{LLM}) de pesos abiertos responder a las consultas del usuario utilizando exclusivamente información extraída de la web oficial de la universidad.

El desarrollo del proyecto abarca un flujo completo de ingeniería de datos e inteligencia artificial: desde la minería de datos mediante técnicas de \textit{web scraping} y la división semántica de la información (\glr{chunking}{chunking}), hasta la generación de representaciones vectoriales (\glr{embeddings}{incrustacion}), su indexación en una \glr{base de datos vectorial}{base de datos vectorial} y la integración final con el LLM a través de una interfaz web accesible. Todo ello se somete a una evaluación experimental con métricas objetivas de fidelidad (\textit{faithfulness}) y relevancia de las respuestas para asegurar que el sistema no incurra en la invención de datos (alucinaciones).
...
Para garantizar la viabilidad, el rigor científico y la completitud del proyecto dentro de las restricciones de tiempo y carga lectiva, se establecen límites precisos en tres subapartados fundamentales: el dominio de la información a utilizar, el público objetivo y la arquitectura tecnológica.
...
Dentro de esta delimitación, se excluirá además del corpus la modalidad en extinción de una titulación cuando coexista con un plan de estudios vigente que la sustituye, como pasa con la titulación de Ingeniería Geomática y Topográfica, \sout{en el que un estudiante preuniversitario} \textcolor{borrador2}{en la que un estudiante preuniversitario} no puede matricularse en un plan que ya no admite nuevo alumnado, por lo que incluirlo entre las recomendaciones del sistema induciría a error.

\subsection{Público objetivo y profundidad de la información}
El sistema está diseñado para orientar a estudiantes preuniversitarios (Educación Secundaria, Bachillerato y Formación Profesional) que se encuentran en fase de búsqueda y decisión sobre su futura carrera universitaria. Por este motivo, la extracción de datos de las \glr{guías docentes}{guía docente} \sout{se han acotado a:} \textcolor{borrador2}{se ha acotado a:}
\begin{itemize}        
...
\begin{borrador}
  \item \textbf{Una frase por revisar en la delimitación del dominio:} <<El sistema no abarcará
  la oferta de másteres o títulos propios de la EPSJ, así como de las titulaciones\dots>>. Tras
  una negación, <<así como>> afirma lo contrario de lo que se quiere decir; lo que enlaza dos
  elementos negados es <<ni>>.
\end{borrador}
