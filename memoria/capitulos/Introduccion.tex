...
\sout{En este capítulo se realizará un acercamiento al tema de investigación del que trata este trabajo final de grado (TFG), en el que se plantea el desarrollo de un sistema de recomendación e información académica centrado en la oferta de la Escuela Politécnica Superior de Jaén (EPSJ).}
...
\begin{borrador}
    \item \textbf{Fase:} 5 (cierre) --- diferido por decisión propia (respondida al tutor arriba): el resumen y el diagrama de bloques se redactan cuando el sistema esté avanzado y los bloques sean definitivos.
    \item Posible esqueleto del resumen de una página: (1) una frase con el problema (barrera de acceso a la información académica); (2) una frase con la solución construida (web de chat con RAG sobre un LLM local); (3) un párrafo breve con las tecnologías empleadas por fase (Scrapy para la extracción, el modelo de incrustaciones y base vectorial que se elijan en el Capítulo~\ref{cap:materialesymetodos}, el LLM local, la aplicación web); (4) el diagrama de bloques; (5) una frase de cierre con el estado actual del proyecto (qué fases están completas).
    \item Ojo con la norma 2 del tutor (impersonal reflexivo) también aquí: <<se ha construido>>, no <<hemos construido>>.
\end{borrador}
