...
El presente Trabajo Fin de Grado aborda el diseño, la implementación y la evaluación de un sistema de recomendación de titulaciones para la Escuela Politécnica Superior de Jaén (EPSJ) de la Universidad de Jaén (UJA): un agente conversacional basado en procesamiento del lenguaje natural (PLN) que asiste a los estudiantes de secundaria proporcionando respuestas precisas, contextualizadas y en lenguaje natural sobre las titulaciones del centro.

El sistema se articula sobre una arquitectura de Recuperación Aumentada por Generación (RAG, por sus siglas en inglés: \textit{Retrieval-Augmented Generation}), que permite a un modelo de lenguaje grande (\glr{LLM}{LLM}) de código abierto responder a las consultas del usuario utilizando exclusivamente información extraída de la web oficial de la universidad.

El desarrollo del proyecto abarca un flujo completo de ingeniería de datos e inteligencia artificial: desde la ingesta de datos mediante técnicas de \textit{web scraping} y la división semántica de la información (\textit{chunking}), hasta la generación de representaciones vectoriales (\textit{embeddings}), su indexación en una base de datos vectorial y la integración final con el LLM a través de una interfaz web accesible. Todo ello se somete a una evaluación experimental con métricas objetivas de fidelidad (\textit{faithfulness}) y relevancia de las respuestas, para asegurar que el sistema no incurra en la invención de datos (alucinaciones).

\begin{borrador}
    \item \textbf{Fase:} 0 --- el resumen ya está redactado y depurado de duplicidades con la Sección 1.1 (la motivación vive solo allí; aquí solo el qué y el cómo, en presente descriptivo). Queda únicamente el diagrama de bloques del conjunto del TFG (se puede generar con el mismo procedimiento de doble versión que las demás figuras).
\end{borrador}
