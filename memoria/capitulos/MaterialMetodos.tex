...
\begin{itemize}                                                                                                                                  
  ...
  \item \textbf{Ramas efímeras:} Cada nueva característica, corrección o experimento se desarrolla en una rama aislada de un solo uso (ej. \texttt{IT-XX-descripcion}).
        ...
        \textcolor{borrador2}{La Figura~\ref{fig:git_flow} recoge esta estrategia: cada rama efímera nace siempre de \texttt{main} actualizado y vuelve a él mediante una confirmación de fusión, de modo que la rama \texttt{doc} avanza en paralelo sin bloquear el desarrollo del código.}
...
\begin{borrador}
  \item \textbf{Fase:} 5 (IT-52) --- cifras a refrescar al cerrar la memoria.
        El \textit{backlog} ha vuelto a crecer y las cifras del texto se han
        quedado atrás. A 23/07/2026 el repositorio tiene \textbf{64} historias
        (IT-01 a IT-65, sin IT-18, que se dividió en IT-63 e IT-64) con un
        reparto de \textbf{38} \textit{must}, \textbf{21} \textit{should} y 4
        \textit{could}. Hay que actualizar el <<61>> y el <<IT-01 a IT-61>> que
        aparecen en tres sitios (Secciones 4.1.1, 4.2.1 y en el párrafo de
        arriba) y el reparto MoSCoW de este mismo párrafo. Las filas de IT-62 e
        IT-65 ya se han añadido a la Tabla~\ref{tab:backlog}.
  \item Como esto va a volver a pasar cada vez que se añada una tarjeta,
        conviene dejar el refresco de estas cifras como último paso de IT-52 y
        no ir corrigiéndolas a mano en cada fase.
  \item La Tabla~\ref{tab:backlog} tiene ahora un tipo nuevo, DQA, que solo usa
        IT-65; comprobar al cerrar que el capítulo lo ha explicado antes de que
...
\begin{borrador}                                                                                                                                  
  ...
  \item \textbf{Deuda detectada el 23/07/2026.} IT-30 (canalización de
        indexación) e IT-27 (conjunto de evaluación) están cerradas y su código
        está fusionado en \texttt{main} (\texttt{src/tfg\_uja/indexer.py} y
        \texttt{eval/preguntas\_evaluacion.json}, con 36 preguntas etiquetadas),
        pero ni una ni otra aparecen todavía en la memoria. Eso incumple el
        sexto criterio de la Definición de Hecho que se enuncia en la
        Sección~\ref{secc:dod} de este mismo capítulo, así que hay que cubrirlo
        en IT-33: una sección de Fase 1 que describa la canalización de
        indexación y, en el Capítulo~\ref{cap:resultados}, el diseño del
        conjunto de evaluación.
  \item Ojo al escribirla: el modelo de incrustaciones y la base vectorial que
        usa hoy \texttt{indexer.py} son provisionales por decisión explícita
        (quedan fijados en IT-29/ADR-0003 e IT-32/ADR-0004), así que no se
        pueden presentar como la elección final del sistema.
\end{borrador}
...
\textcolor{borrador2}{Cada una de estas anomalías, con la evidencia real que la respalda y el tratamiento aplicado en el código, se documenta en el registro DQA-0001\cite{repo_tfg}; aquí se recogen solo a alto nivel para no desviar la memoria hacia el detalle de problemas puntuales.}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.5cm} Y c@{}}
    ...
    Cortar donde cae la similitud entre frases obligaría a elegir ya el modelo de incrustaciones, 
    que es una decisión de la Fase 1. Prematura.                                                                                                                                                                              & 
    Descartada                                                                                                                                                                                                                                                                                    \\
    \addlinespace
...
\begin{borrador}
  \item \textbf{Fase:} 1 (IT-28/IT-29). Estos tres valores se fijaron por criterio, no por experimento. Cuando se elija el modelo de embeddings hay que revisar el tamaño máximo contra su ventana de contexto real y contrastar objetivo/mínimo con las métricas de recuperación del experimento comparativo, en vez de dejarlos como están por costumbre.
\end{borrador}
