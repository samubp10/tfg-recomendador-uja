...
\section{Tecnologías y herramientas}
 ...
 Como lenguaje base he utilizado Python en su versión 3.13, y para la extracción de datos \sout{habían 3 opciones clara que eran las que más se habituaban a nuestro contexto:} \textcolor{red}{se han estudiado tres claras opciones, las cuales se comentan de los apartados siguientes.}

\subsubsection{Scrapy\cite{scrapy}}

 Scrapy\footnote{\url{https://scrapy.org/}} es un \textit{framework} asíncrono completo de \textit{crawling} (rastreo web). \textcolor{red}{Dadas sus características, las cuáles se detallan a continuación, ha sido la opción elegida.}

\sout{Las características principales que justificaron su elección son:}
...
\begin{itemize}
    ...
    \item \textbf{Control de tasa (Throttling):} El \textit{framework} ofrece parámetros de configuración para permitir ajustar el proceso de rastreo para no saturar nuestro servidor objetivo.
...
\textcolor{red}{Conforme a lo comentado anteriormente, se} adopta \textbf{Scrapy} \textcolor{red}{como herramienta de extracción de datos de webs}. La evidencia sobre la gestión correcta del \textit{encoding} fue el factor decisivo, reforzado por el soporte nativo de \texttt{robots.txt}, el \textit{throttling} configurable, la testabilidad \textit{offline} con \texttt{HtmlResponse} y la gran comunidad de soporte. Las alternativas analizadas requerían reinplementar manualmente dichas garantías o estaban sobredimensionadas para un sitio estático.
...
Con estos trozos de información, se pasa al \textcolor{red}{embedding} con el que \sout{meteremos} \textcolor{red}{se introducirán} los datos en la Base de datos vectorial.

%OJO te faltaba una "d" en embedding. 

\sout{Y ya }Por último llega el \textit{pipeline} RAG en el que después de la pregunta del usuario, el propio sistema utilizando como memoria la base de datos vectorial, recuperará los \textit{chunks} pertenecientes a la pregunta del usuario y responderá. Toda esta última fase se podrá interactuar mediante la aplicación web
