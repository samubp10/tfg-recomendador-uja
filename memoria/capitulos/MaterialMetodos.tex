...
\begin{borrador}
  ...
  \item \textbf{Los seis criterios están redactados para tarjetas de código, pero el texto
    dice que se aplican a todas y encima <<simultáneamente>>.} Contado el 05/08/2026 con
  \texttt{gh} sobre las 99 tarjetas del repositorio: 34 son de código, 42 de memoria, 10 de
  experimento, 5 de ADR, 6 de gestión y 2 de DQA. Es decir, \textbf{65 de 99 no tienen
    código}, así que no pueden cumplir el criterio 1 (<<el código cumple el objetivo>>), ni el
  2 (<<casos de prueba>>), ni el 3 (<<\textit{docstrings}>>). La afirmación es comprobable y
  hoy es falsa para dos tercios del \textit{backlog}: basta con abrir IT-33, que es un
  capítulo de la memoria, y preguntar dónde están sus pruebas.
  ...
  \item \textbf{Hay además dos incumplimientos reales del criterio 6, y conviene declararlos}
  en lugar de esperar a que los encuentren. IT-27 e IT-30 se cerraron y se fusionaron sin
  volcar su contenido en la memoria. No es una sospecha: lo certifica el propio tablero,
  porque las tarjetas abiertas que lo saldan se titulan literalmente <<deuda de IT-27>> y
  <<deuda de IT-30>>. Declararlo es más fuerte que ocultarlo, porque demuestra que el proceso
  se aplica de verdad: solo se detecta un incumplimiento allí donde alguien comprueba.
\end{borrador}
...
\subsection{Por qué un modelo de pesos abiertos}
...
\textcolor{borrador2}{Todo el sistema ---extracción, fragmentación, indexación, recuperación
  y generación--- se ejecuta y se demuestra sobre un único equipo portátil doméstico, cuyas
  características recoge la Tabla~\ref{tab:equipo}. Esa máquina no es un detalle de
  implementación: es una restricción de diseño que aparece enunciada en la hipótesis de
  trabajo (Sección~\ref{secc:hipotesis}) y que ya ha condicionado decisiones tomadas, entre
  ellas la del modelo de incrustaciones.}

\textcolor{borrador2}{El dato que más pesa no es la velocidad del procesador, sino los 16 GB
  de memoria principal, porque en la Fase 2 el modelo de recuperación y el modelo generativo
  tendrán que convivir en ella. De ahí que la comparación entre el modelo pequeño y el grande
  (Sección~\ref{secc:eleccion_e5small}) acabara resolviéndose por memoria residente y no por
  calidad de la recuperación.}

\textcolor{borrador2}{Queda por declarar una limitación que resulta contraintuitiva al leer
  la tabla: el equipo tiene una tarjeta gráfica con 6 GB de memoria dedicada, pero
  \textbf{el proyecto no la utiliza}. El entorno de ejecución monta la compilación de PyTorch
  destinada exclusivamente a procesador ---comprobado el 05/08/2026:
  \texttt{torch.cuda.is\_available()} devuelve falso---, de modo que todos los tiempos que
  aparecen en esta memoria son de cómputo en CPU. La tarjeta está físicamente disponible y
  aprovecharla es trabajo futuro, no una capacidad que el sistema tenga hoy.}
