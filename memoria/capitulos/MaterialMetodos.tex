...
\subsubsection{Las dos estrategias sobre el mismo texto}

\textcolor{borrador2}{Los motivos de descarte de la Tabla~\ref{tab:alternativas_chunking} se
  enuncian ahí, pero conviene verlos ocurrir. El guion
  \texttt{scripts/ejemplo\_fragmentacion.py} aplica las dos estrategias al mismo material real
  ---las guías de \emph{Automática avanzada} y \emph{Creación de empresas de base
    tecnológica}, consecutivas en la colección--- y es determinista: no consulta la web ni
  carga ningún modelo, de modo que el ejemplo puede regenerarse cuando la colección cambie en
  lugar de quedarse citando un texto que ya no existe.}

\textcolor{borrador2}{Troceando por tamaño fijo, el primer fragmento termina así:}

\begin{quote}
  \small\ttfamily
  [\ldots] Componentes de un Grafcet según la norma 61131-3. Traducción a lenguaje
\end{quote}

\textcolor{borrador2}{La frontera cae donde le toca al contador de caracteres, no donde acaba
  la frase. No es mala suerte: de las siete fronteras internas que genera esta estrategia sobre
  el ejemplo, \textbf{seis cortan a mitad de frase}. La estrategia elegida produce
  \textbf{cero de dos} sobre el mismo contenido.}

\textcolor{borrador2}{El segundo defecto es más grave, porque incumple una restricción de
  diseño y no una preferencia de calidad. El tercer fragmento contiene esto:}

\begin{quote}
  \small\ttfamily
  [\ldots] 14. Control de una estación de ensamblado de piezas 3

  \textcolor{borrador2}{\textrm{\textbf{$\leftarrow$ aquí acaba una asignatura y empieza otra}}}

  Conocimientos previos y recomendaciones. La creación de nuevas iniciativas empresariales
  [\ldots]
\end{quote}

\textcolor{borrador2}{Un único fragmento mezcla el temario de una asignatura de automática con
  los requisitos previos de una de creación de empresas. Recuperado en respuesta a una pregunta
  sobre cualquiera de las dos, el modelo generativo recibiría material de la otra sin ninguna
  señal de que no le corresponde. La estrategia estructural no puede incurrir en este error,
  porque no parte del texto continuo sino de la unidad: empieza por la guía completa de una
  asignatura y solo la divide si excede el máximo.}

\textcolor{borrador2}{Sobre ese mismo contenido, el primer fragmento de la estrategia elegida
  empieza y acaba así:}

\begin{quote}
  \small\ttfamily
  «Automática avanzada», asignatura optativa de 6 ECTS del Grado en Ingeniería de Organización
  Industrial. Conocimientos previos y recomendaciones. El alumno debería haber aprobado la
  asignatura de Automática Industrial. [\ldots] Programación en lenguaje KOP. Recordatorio de
  automatismos programados.
\end{quote}

\textcolor{borrador2}{Cierra en una frontera del texto y, sobre todo, arranca con un
  encabezado que nombra la asignatura, su carácter, sus créditos y la titulación. Ese
  encabezado es lo que permite que el fragmento siga significando algo cuando se recupera
  aislado de los demás, que es exactamente la situación en la que va a llegarle al modelo
  generativo.}
