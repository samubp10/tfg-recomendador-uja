...
El ciclo de vida del proyecto se ha gestionado mediante un tablero \textbf{Kanban} virtual (implementado en \textit{GitHub Projects}), dividiendo el alcance total en \sout{61 unidades de trabajo atómicas o historias de usuario (etiquetadas como IT-01 a IT-61)} unidades de trabajo atómicas o historias de usuario. Estas tareas se agrupan secuencialmente en seis fases lógicas (Fase 0 a Fase 5), desde la minería de datos hasta la validación con usuarios reales.

\begin{borrador}
  \item \textbf{Corrección de cifras (30/07/2026).} Contadas con \texttt{gh} sobre
        el repositorio: son \textbf{97} historias, de \textbf{IT-01 a IT-98} (el
        número IT-18 no llegó a existir). 52 cerradas y 45 abiertas.
  \item Esta cifra aparece \textbf{tres veces} en el capítulo (aquí, en la
        micro-planificación y en la priorización). Al corregirla hay que cambiar
        las tres o volverán a discrepar entre sí.
  \item Conviene decidir cómo contarlo antes de escribirlo: el \textit{backlog} ha
        crecido un 59~\% durante el desarrollo, y eso no es un descuido que
        esconder sino una consecuencia de trabajar con Kanban sobre una fuente que
        cambia. Dicho así es un argumento a favor de la metodología elegida; dicho
        como una cifra que no cuadra, es un despiste.
\end{borrador}
