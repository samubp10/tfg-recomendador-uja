...
\small
\begin{xltabular}{\textwidth}{@{}l X l c c@{}}
...
\begin{borrador}
    \item \textbf{Fase:} 0 (IT-08, IT-09 e IT-56) --- redactable ya; al escribirla se completa también la referencia desde la sección <<Arquitectura del sistema>>.
    \item La unidad de troceado es la asignatura (su guía) o el bloque de salidas: \textbf{un fragmento nunca mezcla dos asignaturas} (decisión innegociable del proyecto).
    \item Algoritmo: división por párrafos; si un trozo sigue excediendo el máximo, división por frases; después, fusión de residuales pequeños con reequilibrio.
    \item Cada fragmento lleva un encabezado autocontenido (asignatura, tipo, ECTS, grado) para que tenga sentido por sí solo al recuperarlo.
    \item Deduplicación por la clave (nombre, contenido): solo por contenido fusionaría asignaturas distintas con el mismo texto de respaldo (caso real: Smart Grids y Técnicas de ingeniería gráfica).
    \item Cifras verificadas del dataset: 1\,172 fragmentos antes de deduplicar, 892 después (un 24\,\% menos), 28 unidades compartidas entre titulaciones, tamaños mín/mediana/máx 227/1\,093/1\,499 caracteres. Máximo de 1\,500 como restricción dura; mínimo de 200 como preferencia.
    \item Dejar claro que los parámetros de fragmentación son \textbf{provisionales} hasta el experimento de la Fase 1: no presentarlos como óptimos.
    \item Aquí va también el diagrama de secuencia del \textit{chunking}, ya generado en la carpeta de diagramas de las notas del proyecto y pendiente de subir a Overleaf e insertar.
\end{borrador}
...
\section{Fase 1: indexación vectorial y conjunto de evaluación}

\begin{borrador}
    \item \textbf{Fase:} 1 (IT-33 con el material de IT-30 e IT-27, ya implementados y fusionados) --- el código va por delante de la memoria a propósito; esta sección se redacta con hechos ya verificados, no con planes.
    \item \textbf{Pipeline de indexación (IT-30):} el módulo \texttt{indexer.py} convierte los 892 fragmentos en vectores por lotes y los persiste con sus metadatos en una colección de ChromaDB. Puntos defendibles: (a) el índice se reconstruye completo en cada ejecución porque es un artefacto derivado, igual que \texttt{chunks.json} (mismo argumento de reproducibilidad); (b) las listas paralelas grados/códigos se serializan como texto porque la base solo admite metadatos escalares; (c) el código ausente de las salidas queda vacío, fiel a la política de no imputar.
    \item \textbf{Elecciones provisionales, no presentarlas como óptimas:} el modelo de \textit{embeddings} por defecto (\texttt{paraphrase-multilingual-MiniLM-L12-v2}: multilingüe, ejecutable en CPU doméstica) y la propia ChromaDB son valores de trabajo hasta los experimentos IT-28/IT-31 y sus ADR-0003/0004. El incrustador es un parámetro inyectable justo para que el experimento intercambie modelos sin tocar el módulo.
    \item \textbf{Evidencia ya obtenida (verificada por ejecución):} 892/892 fragmentos indexados; en la consulta de humo <<¿en qué asignaturas se estudia inteligencia artificial?>> la guía correcta sale primera; en cambio, la consulta de salidas profesionales NO devuelve el bloque de salidas en el top-3. Este segundo resultado es material de oro: motiva empíricamente el experimento de selección de modelo en vez de darlo por bueno.
    \item \textbf{Conjunto de evaluación (IT-27):} 36 preguntas reales en lenguaje de estudiante (salidas 8, temario 18, metadatos 6, sin guía 4), cubriendo los 8 grados. Decisión central a redactar: las unidades relevantes se anotan por selector estable (origen, nombre, grado) y no por id de fragmento, porque los ids son posicionales y cambian al re-fragmentar --- el patrón oro no debe caducar cuando la Fase 1 experimente con otros parámetros de \textit{chunking}. Validación en dos niveles: estructura en CI y resolución contra el dataset completo en local (\texttt{check\_evalset.py}).
    \item Diagramas ya generados en la carpeta de diagramas de las notas (pendientes de doble versión TikZ e inserción aquí): secuencia de la indexación y flujo del conjunto de evaluación con Recall@K/MRR.
    \item Al redactar, definir aquí solo el PROCESO; las métricas (Recall@K, MRR) se definen en el Capítulo~\ref{cap:antecedentes} (subsección diferida a Fase 1, ahora desbloqueada) y los RESULTADOS de los experimentos van al Capítulo~\ref{cap:resultados}.
\end{borrador}