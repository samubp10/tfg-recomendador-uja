...
\begin{borrador}
  \item \textbf{Fase:} 1 (tutoría del 04/08/2026: <<diferenciar tests de código y las
        ...
        tus palabras.
  \item \textbf{Falta un número aquí y no lo pongo yo inventado:} cuántas pruebas tiene hoy
        la batería. Cuéntalo con \texttt{pytest} sobre \texttt{main}, no sobre \texttt{doc}
        --- esta rama va por detrás y da una cifra menor de la real.
...
\begin{borrador}
  \item \textbf{Cifras contadas con \texttt{gh} el 04/08/2026}: 98 historias, 54 cerradas
        y 44 abiertas; 65 \textit{must}, 27 \textit{should} y 5 \textit{could}. Hay que
        volver a contarlas al cerrar la memoria, porque el \textit{backlog} sigue vivo.
  \item \textbf{Dos etiquetas del repositorio están incompletas} y por eso los números de
        arriba no salen solos de \texttt{gh}: IT-55 no tiene ni fase ni tipo (aquí se
        cuenta como Gestión de la Fase 0, que es como aparecía en la tabla anterior) e
        IT-100 tiene dos tipos a la vez, código y experimento (aquí se cuenta una sola
        vez, como código) y no tiene prioridad. Son dos clics en GitHub, pero hazlos tú:
        el \textit{backlog} es tuyo.
...
\begin{borrador}    
  ...
  \item \textbf{Hay dos decisiones abiertas} que no puedes dejar sin resolver antes de
  entregar, y que son tuyas: (1) ratificar \texttt{e5-small} o cambiar a
  \texttt{e5-large}, con la justificación de coste que está en la adenda del ADR-0003;
  (2) decidir si el \emph{listado} de asignaturas se presenta como parte del diseño de la
  colección (Fase~0) o como una corrección posterior (Fase~1). Hoy está contado como lo
  segundo.
\end{borrador}
