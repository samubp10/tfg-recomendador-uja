...
\begin{borrador}
  \item \textbf{Revisión de estructura (08/08/2026).} Dos de las ocho secciones de este
  capítulo están vacías y son las que responden a un objetivo: la de la colección
  (OE1) y la de la discusión. Mientras sigan vacías, el criterio de evaluación que exige la
  correspondencia entre los objetivos y la discusión de resultados no se puede cumplir, y es
  un criterio con el que se puntúa.
...
\begin{borrador}
  \item \textbf{Sección nueva, por escribir (07/08/2026).} Es la que responde al \textbf{OE1}
  y hoy no existe, de modo que el primer objetivo del trabajo no tiene resultado en el
  capítulo donde el tribunal lo va a buscar.
  \item Qué tiene que llevar: cuántas titulaciones, asignaturas, guías y bloques de salidas
  tiene la colección; la cobertura de guías y su sesgo; cuántas asignaturas se quedan sin
  contenido y por qué; y las anomalías de la fuente que se encontraron por el camino. Las
  cifras salen de \texttt{check\_dataset.py} y \texttt{check\_chunks.py}, no de la memoria.
  \item \textbf{Aquí NO va} cómo se extrajo ---eso es el Capítulo~\ref{cap:materialesymetodos}---
  sino qué salió.
  \item \textbf{Se ha retirado la sección <<Métricas de evaluación de la recuperación>>} que
  ocupaba este sitio y estaba vacía: lo que iba a explicar ---qué son la exhaustividad, el
  MRR y los demás--- ya está en el Capítulo~\ref{cap:antecedentes}, que es donde va lo que
  existe. Si la echas de menos, dímelo y la repongo.
\end{borrador}
...
\begin{borrador}
  \item \textbf{Los resultados de la rejilla se han traído aquí desde el
    Capítulo~\ref{cap:materialesymetodos} el 07/08/2026.} Allí se quedó el diseño del
  experimento ---qué se compara, cómo y por qué con el mismo número de intentos cada una---
  y aquí vienen las cifras, que es el reparto que ya se seguía con las incrustaciones y el
  que exige la estructura de la memoria.
  \item Con eso desaparece además el problema que lo motivó: el Capítulo~4 ya no enuncia
  ningún resultado antes del experimento que lo produce.
  \item Esta sección responde al \textbf{OE2}. Al reescribirla, comprueba que se lea así:
  que la fragmentación quedó justificada sobre la medición de los textos y el contraste de
  alternativas, que es literalmente lo que ese objetivo promete.
\end{borrador}
