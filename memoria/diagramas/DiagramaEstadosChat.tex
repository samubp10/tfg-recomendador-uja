% Los tres estados de la pantalla de chat (IT-43).
%
% NADA de lo que se dibuja aqui esta inventado:
%   - Las preguntas y las respuestas salen de una conversacion ejecutada contra
%     el sistema el 24/08/2026, con sus tiempos medidos.
%   - Los textos de la espera son literalmente los que compone `contarLaEspera`
%     en la interfaz, incluido el umbral a partir del cual explica por que
%     tarda.
%   - El pie «respuesta inmediata» es la marca que la interfaz pone cuando la
%     respuesta no llego a consultar al modelo.
%
% No se dibuja bloque de fuentes: el sistema no cita fragmentos en la
% respuesta, y dibujarlo prometeria algo que no hace.
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      \begin{scope}[shift={(8.6,0)}]
        ...
        {La cuenta empieza en «Buscando en las guías docentes» y cambia a este
          texto a los 12 s. Mediana medida: 62,7 s; percentil 90: 125,3 s.};
        \node[entrada, text width=6.5cm, anchor=north west] at (0.3,1.5)
        {Escribe tu pregunta\ldots};
        \node[rotulo] at (3.8,-0.2) {2 · La espera};
      \end{scope}

      % ============ 3. INMEDIATA Y SIN INFORMACION ============
  ...
  \caption[\textcolor{borrador2}{Los tres estados de la pantalla de chat}]{\textcolor{borrador2}{Los
      tres estados que el diseño de la pantalla tiene que resolver, además del
      caso corriente. Las preguntas y las respuestas proceden de una
      conversación ejecutada contra el sistema, y los tiempos son los que tardó
      en darlas; los textos de la espera son los que compone la propia interfaz.
